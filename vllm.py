...
import random
import numpy as np
from fedlogger.logbooker import Attender
...
def fl_finetune(
        ...
        # heterogeneous
        heter: bool = False,  
        local_ranks: List[int] = [32, 32, 16, 16, 8, 8, 4, 4, 4, 4],
        dataset_name = 'recap',
        mini: bool = True,
        algo: str = 'Hetlora',
        missing_rate = 0.4,
    ...
    set_random_seed(666)
    index = f'{dataset_name}_{algo}_{missing_rate*100}'
    logger = Attender(
        index = index,
        filePath = f'logs/{index}.log'
    )
    ...
    for epoch in tqdm(range(num_communication_rounds)):
        ...
        for client_id in selected_clients_set:
            ...
            if per_test and epoch == len(num_communication_rounds) - 1:
                # save personalized model
                c_model.save_pretrained(
                    os.path.join(output_dir, index + '_' + str(client_id)),
                    load_in_8bit=False,
                    torch_dtype=torch.float32,
                    device_map=device_map
                )
        ...
        if epoch == len(num_communication_rounds) - 1:
            ...
            print('Acc of Epoch', str(epoch), 'Global acc is:', acc)
            acc_list.append(acc)
            del g_model
            if per_test:
                test_client_id = client_id if dataset_name not in dataset_test_format else dataset_test_format[dataset_name]
                prev = None
                for client_id in selected_clients_set:
                    # c_model = PeftModel.from_pretrained(
                    #     model,
                    #     os.path.join(output_dir,f'{index}_{client_id}'),  
                    #     is_trainable=False       
                    # )
                    if prev is not None:
                        model.delete_adapter(prev)
                    lora_dir = os.path.join(output_dir,f'{index}_{client_id}')
                    model.load_adapter(lora_dir, adapter_name=str(client_id), is_trainable=False)
                    # model.load_adapter()
                    client_acc = llava_evaluation(
                        epoch = epoch, 
                        client_id = test_client_id, 
                        model = c_model, 
                        processor = processor, 
                    ...
                    prev = str(client_id)
                    client_acc_list[client_id] = client_acc_list
                logger.info(client_acc_list)
