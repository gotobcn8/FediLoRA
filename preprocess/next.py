...
# class IamDataset(HuggingFaceDataset):
#     def __init__(self, images_path, recaps_path, clients_num=10, train_test_ratio=0.85):
#         super().__init__(
#             dataset_name="alpayariyak/IAM_Sentences_LLaVA_json", 
...
#             'sid': sid,
#             'id': item['id'],
#             'image': os.path.join(self.images_path,f"{item['id']}.jpg"),
#             'conversations': item['conversations']
#         }
#         return new_sample
    

class NextDataset(HuggingFaceDataset):
    def __init__(self, images_path, recaps_path, clients_num=10, train_test_ratio=0.85):
        super().__init__(
            dataset_name="battleMaster/llava-next-Preference-dataset-20k", 
            images_path=images_path,
            recaps_path=recaps_path,
            clients_num=clients_num,
            train_test_ratio=train_test_ratio)
...
if __name__ == "__main__":
    ...
    next_images_path = '/workspace/2025EMNLP-FLME/input_next/images'
    next_recaps_path = '/workspace/2025EMNLP-FLME/input_next/data_recaps'
    ...
    mini = False
    # ------------------------------------------------------------------------
    os.makedirs(next_images_path, mode=mode, exist_ok=True)
    os.makedirs(next_recaps_path, mode=mode, exist_ok=True)
    new_dataset = NextDataset(
        images_path=next_images_path,
        recaps_path=next_recaps_path,
